  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={`fixed left-0 top-0 z-50 flex h-screen flex-col overflow-hidden bg-[#09090b] ${isMobile && isCollapsed ? "border-none" : "border-r border-[#27272a]"
          }`}
      >
        {/* Top Section */}
        <div className="p-4">
          ...
          <button
            className={`flex items-center gap-3 rounded-full bg-[#1a1a1a] text-zinc-200 transition-all hover:bg-[#27272a] ${isCollapsed ? "justify-center p-3" : "w-full px-4 py-3"
              }`}
          >
        ...
        <div className="custom-scrollbar flex-1 overflow-y-auto overflow-x-hidden px-3 py-2">
          ...
          <div className="space-y-1">
            {history.map((item) => (
              <button
                key={item.id}
                className={`group flex w-full items-center gap-3 rounded-lg p-2 text-zinc-400 transition-colors hover:bg-zinc-800/50 hover:text-zinc-100 ${isCollapsed ? "justify-center" : "text-left"
                  }`}
