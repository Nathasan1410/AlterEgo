...
export default function PostGeneratorWizard() {
    ...
    // Score State
    const [viralScore, setViralScore] = useState<{ score: number, breakdown: any } | null>(null);
    ...
    const handleConfirmPolish = async () => {
        ...
        try {
            ...
            if (data.result) {
                ...
                // Calculate Score
                const scoreResult = calculateTotalScore(deck.hook, deck.body, deck.cta);
                setViralScore(scoreResult);
            }
        } catch(e) { console.error(e); }
        setLoading(false);
    };
    ...
    const handleRePolish = async () => {
        ...
        try {
            ...
            if (data.result) {
                setDeck(prev => ({ ...prev, final: data.result }));
                 // Recalculate Score (assuming polished text might be better, though strict breakdown uses components)
                 // For now, re-use component score but you could parse the final text if needed.
                 // Let's stick to component score for consistency or parse if we had a parser.
                 // Simple approach: Recalculate based on original components (since polish mainly changes style)
                 // OR better: Assume Polish improves it slightly (+5 boost) for effect? No, let's keep it honest.
                 const scoreResult = calculateTotalScore(deck.hook, deck.body, deck.cta);
                 setViralScore(scoreResult);
            }
        } catch(e) { console.error(e); }
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        {viralScore && (
                            <motion.div 
                                ...
                                className="bg-zinc-900 border border-zinc-700 p-4 rounded-xl flex items-center gap-4 shadow-lg"
                            >
                                <div className="text-center">
                                    <div className="text-3xl font-black text-white">{viralScore.score}</div>
                                    <div className="text-[10px] uppercase tracking-wider text-gray-400 font-bold">Viral Score</div>
                                </div>
                                <div className="h-8 w-px bg-zinc-700"></div>
                                <div className="space-y-1 text-xs text-gray-300">
                                    <div className="flex justify-between gap-4"><span>Hook:</span> <span className={viralScore.breakdown.hook > 80 ? 'text-green-400' : 'text-yellow-400'}>{viralScore.breakdown.hook}%</span></div>
                                    <div className="flex justify-between gap-4"><span>Body:</span> <span className={viralScore.breakdown.body > 80 ? 'text-green-400' : 'text-yellow-400'}>{viralScore.breakdown.body}%</span></div>
                                    <div className="flex justify-between gap-4"><span>CTA:</span> <span className={viralScore.breakdown.cta > 80 ? 'text-green-400' : 'text-yellow-400'}>{viralScore.breakdown.cta}%</span></div>
                                </div>
                            </motion.div>
                        )}
                    </div>
        ...
        return (
            <div className="w-full max-w-4xl mx-auto p-4 z-10 relative">
                <div className="bg-white dark:bg-gray-800 rounded-xl p-8 shadow-2xl border border-gray-100 dark:border-gray-700">
                    ...
                    {/* Opik Badge */}
                    <div className="mb-4 inline-flex items-center gap-2 px-3 py-1 bg-orange-100 dark:bg-orange-900/30 rounded-full text-xs font-medium text-orange-700 dark:text-orange-300">
                        <span className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></span>
                        Traced by Opik AI
                    </div>
                    
                    <pre className="whitespace-pre-wrap font-sans text-gray-700 dark:text-gray-200 text-lg leading-relaxed bg-gray-50 dark:bg-gray-900 p-6 rounded-xl">
                        {deck.final || 'Polishing your masterpiece...'}
                    </pre>
                    <div className="mt-6 flex gap-4 flex-wrap">
                        <Button onClick={() => { navigator.clipboard.writeText(deck.final || '') }}>Copy Text</Button>
